...
export class SvgComponent implements OnInit, OnDestroy {
  ...
  // Set the interval to a variable so you can stop it later
  private warpTimeout = null;
  private path = undefined;
  
  // make the SVG Array available to the other funcitons
  private svgArray = [
  ...
  createSvg() {
    ...
    this.path = svgCanvas.path(this.svgArray[0]);

  }
  
  warp(num = 0) {
    // if there's a timeout waiting to warp, clear it so we can start the new warp
    ...
    function step(num) {
      var currentStep = steps[num]
      var path = currentStep.path
      // use `mina.easin` as a default, unless something else is provided
      var easing = currentStep.easing || mina.easin
      this.path.animate(path, easing)
      
      if (currentStep.nextStep) {
        // use `4000` as a default, unless something else is provided
        var timeout = currentStep.timeout || 4000
        setTimeout(() => {
